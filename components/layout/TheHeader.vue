  ...
  <v-app-bar app clipped-left>
    ...
    <v-menu v-if="isAuthenticated" offset-y z-index="200">
      ...
      <v-list>
        ...
        <v-list-item v-if="isSuperuser" @click="manageUsers">
          <v-list-item-icon>
            <v-icon>{{ mdiAccountCogOutline }}</v-icon>
          </v-list-item-icon>
          <v-list-item-content>
            <v-list-item-title>
              {{ $t('user.manageUsers') }}
...
<script>
import { mdiLogout, mdiDotsVertical, mdiMenuDown, mdiHexagonMultiple, mdiAccountCogOutline } from '@mdi/js'
...
export default {
  ...
  data() {
    return {
      ...
      mdiMenuDown,
      mdiHexagonMultiple,
      showManageUsers: false, // Altere para true quando necessário
    }
  },
  ...
  methods: {
    ...
    manageUsers() {
      this.$router.push(this.localePath('/manageUsers')) // WORKING ON
    }
  }
}
</script>
